  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex items-center gap-3 mb-6">
        <Link
          href="/moves"
          className="flex items-center gap-1 text-sm text-if-muted transition-colors"
        >
          <ArrowLeft size={14} />
          Capacités
        </Link>
        <span style={{ color: "var(--color-if-border-hi)" }}>/</span>
        <h1 className="text-xl sm:text-2xl font-bold text-if-text">Maîtres des Capacités</h1>
      </div>

      {/* ── Classic tutors ─────────────────────────────────────────────── */}
      <section className="mb-10">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2" style={{ color: "var(--color-if-text-dim)" }}>
          <MapPin size={16} style={{ color: "var(--color-if-accent)" }} />
          Tuteurs classiques
        </h2>
        ...
        {tutorsLoading ? (
          ...
          <div className="space-y-4">
            {locationGroups.map(({ nameEn, locationFr, tutors: group }) => (
              <div key={nameEn} className="rounded-lg border border-if-border-lo overflow-hidden">
                <div className="px-4 py-2 flex items-center gap-2" style={{ background: "var(--color-if-surface)" }}>
                  <MapPin size={13} style={{ color: "var(--color-if-accent)" }} className="shrink-0" />
                  <span className="font-semibold text-if-text-dim text-sm">
                    {locationFr ?? nameEn}
                  </span>
                  {locationFr && (
                    <span className="text-xs text-if-text-xs">({nameEn})</span>
                  )}
                </div>
                <table className="w-full text-sm">
                  <tbody>
                    {group.map((t) => (
                      <tr
                        key={t.id}
                        className="border-t border-if-border-lo hover:bg-if-elevated transition-colors"
                      >
                        ...
                        <td className="px-4 py-2.5 text-right font-mono text-emerald-300 whitespace-nowrap">
                          {formatCurrency(t)}
                        </td>
                        {t.npc_description && (
        ...
        {expertsLoading ? (
          ...
          <div className="space-y-4">
            {expertGroups.map(([location, moves]) => (
              <div key={location} className="rounded-lg border border-if-border-lo overflow-hidden">
                <div className="px-4 py-2 flex items-center gap-2" style={{ background: "var(--color-if-surface)" }}>
                  <MapPin size={13} style={{ color: "var(--color-if-accent)" }} className="shrink-0" />
                  <span className="font-semibold text-if-text-dim text-sm">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-if-surface text-xs text-if-muted">
                  ...
                  <tbody>
                    {moves.map((e) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5 text-if-text-dim text-xs">
