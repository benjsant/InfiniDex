...
export default function FusionsByHeadPage({ params }: { params: Promise<{ headId: string }> }) {
  const { headId } = use(params);
  const hId = parseInt(headId, 10);

  const { data: head, isLoading: headLoading } = usePokemon(hId);
  const { data: bodies = [], isLoading: bodiesLoading } = usePokemonList({
    page_size: 600,
    include_hoenn: true,
  });
  ...
  const filtered = useMemo(() => {
    let list = bodies.filter((b) => b.id !== hId);

    if (typeFilter) {
      const nf = normalize(typeFilter);
      list = list.filter((b) => {
        const t1 = primaryType(b.types);
        const t2 = secondaryType(b.types);
    ...
    return list;
  }, [bodies, hId, typeFilter, sortMode]);

  const totalPages = Math.ceil(filtered.length / PAGE_SIZE);
  const paginated  = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  const headName = head ? (head.name_fr ?? head.name_en) : `#${hId}`;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <nav aria-label="breadcrumb" className="flex items-center gap-2 text-sm text-if-text-xs mb-4">
        ...
        <span>/</span>
        <Link href={`/pokedex/${hId}`} className="hover:text-[#e8b84b] transition-colors">{headName}</Link>
        <span>/</span>
        <span className="text-if-text-dim">en tête</span>
      </nav>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-if-text-hi">{headName} en tête</h1>
        ...
        <div className="flex gap-2 ml-auto flex-wrap">
          ...
          <select
            ...
            style={{ background: "var(--color-if-card)", border: "1px solid var(--color-if-border)", color: "var(--color-if-text-dim)" }}
          >
            <option value="bst_desc">BST corps ↓</option>
            <option value="bst_asc">BST corps ↑</option>
      ...
      {bodiesLoading ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
            {paginated.map((body) => {
              const t1 = primaryType(body.types);
              const t2 = secondaryType(body.types);
              return (
                <Link
                  key={body.id}
                  href={`/fusion/${hId}/${body.id}`}
                ...
                >
                  ...
                  <img
                    src={`/api/sprites/${hId}/${body.id}/image`}
                    alt={`${headName}/${body.name_fr ?? body.name_en}`}
                    width={64}
                    height={64}
                    loading="lazy"
