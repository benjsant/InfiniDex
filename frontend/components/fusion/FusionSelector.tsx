...
function PokemonPicker({ label, selected, onSelect, game, loading = false }: PokemonPickerProps) {
  ...
  return (
    <div className="flex-1 relative" ref={containerRef}>
      ...
      {loading ? (
        <div className="flex items-center gap-3 p-3 rounded-lg bg-if-card border border-if-border-mid animate-pulse">
          <div className="w-12 h-12 rounded bg-if-input shrink-0" />
          <div className="flex-1 space-y-2">
            <div className="h-3 w-24 rounded bg-if-input" />
            <div className="h-3 w-16 rounded bg-if-input" />
          </div>
        </div>
      ) : selected ? (
        <div
          className="flex items-center gap-3 p-3 rounded-lg bg-if-card border border-indigo-500 cursor-pointer hover:border-indigo-400 transition-colors"
          onClick={handleOpen}
        >
          <Image
        ...
        <button
          onClick={handleOpen}
          className="w-full p-3 rounded-lg bg-if-card border border-dashed border-if-elevated text-if-text-xs hover:border-indigo-500 hover:text-indigo-300 transition-all"
        >
          + Choisir un Pokémon
        </button>
      )}
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 z-30 bg-if-card border border-if-border-mid rounded-lg shadow-xl">
          {listError ? (
          ...
          <>
          <div className="p-2 border-b border-if-input">
            <input
              ref={inputRef}
              type="search"
              value={q}
